...
int main ()
{
    string sIn ("/home/ta/tmp/in.fifo");
    string sOut ("/home/ta/tmp/out.fifo");
    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;

    mkfifo(sIn.c_str(),   mode );
    mkfifo(sOut.c_str(),  mode );


    int fIn = open ( sIn.c_str(), O_RDWR);
    int fOut = open ( sOut.c_str(), O_RDWR);

    while (1)
    {
        char buf[128];
        int count = read ( fIn, &buf, sizeof(buf));
        buf[count] = 0;
        if ( count != -1)
        {
            cout<<"buf: "<<buf<<endl;
